...
/// Selects a single peer randomly from the list of specified peers and weights
pub fn choose_random_peer_by_weight(
    peers: Vec<(PeerNetworkId, f64)>,
) -> Result<PeerNetworkId, Error> {
    peers
        .choose_weighted(&mut rand::thread_rng(), |peer| peer.1)
        .map(|peer| peer.0)
