...
impl DataSummaryPoller {
    ...
    fn select_peers_to_poll(
        ...
        let maybe_peers_to_poll = match num_peers_to_poll {
            ...
            num_peers_to_poll => {
                // Select a peer to poll weighted by latency
                let peer_to_poll_by_latency =
                    choose_peer_by_latency(&potential_peers, self.peers_and_metadata.clone());

                // Select the remaining peers randomly
                if let Some(peer_to_poll_by_latency) = peer_to_poll_by_latency {
                    potential_peers.remove(&peer_to_poll_by_latency);
                }
                let random_peers_to_poll =
                    utils::choose_random_peers(num_peers_to_poll - 1, potential_peers);

                // Return all peers to poll
                let peers_to_poll = match peer_to_poll_by_latency {
                    Some(peer_to_poll_by_latency) => random_peers_to_poll
                        .union(&hashset! { peer_to_poll_by_latency})
                        .cloned()
                        .collect(),
                    None => random_peers_to_poll,
                };
                Some(peers_to_poll)
            },
        };
...
pub(crate) fn calculate_num_peers_to_poll(
    ...
    let random_number = rand::thread_rng().gen_range(0, 101) as f64;
    let num_peers_to_poll = if random_number < (num_peers_to_poll.fract() * 100.0) {
...
/// Selects a single peer from the list of potential peers.
/// Peer selection is weighted by peer latencies (i.e., the
...
fn choose_peer_by_latency(
    ...
    match utils::choose_random_peer_by_weight(peer_and_latency_weights) {
        Ok(peer) => Some(peer),
        Err(error) => {
            // Log the error
            sample!(
